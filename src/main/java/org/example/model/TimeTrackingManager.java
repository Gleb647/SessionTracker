...
import org.example.util.VisibleForTesting;
import org.openstreetmap.josm.tools.ListenerList;
import org.openstreetmap.josm.tools.Logging;
import org.example.gui.TimeTrackingUpdateListener;
...
public final class TimeTrackingManager {
     ...
     */
    private long previouslyTrackedSeconds;
     ...
     */
    public void addAndFireTimeTrackingUpdateListener(final TimeTrackingUpdateListener listener) throws MalformedURLException, ApiException.ConnectionFailure {
     ...
     * @return
     */
    public void handlePreviouslyTrackedSeconds(final boolean shouldBeAdded) throws MalformedURLException, ApiException.ConnectionFailure {
        synchronized (this) {
    ...
    private void fireTimeTrackingUpdateListeners(){
        listeners.fireEvent(it -> {
            try {
                it.updateNumberOfTrackedSeconds(
                        committedSeconds +
     ...
     */
    public void trackChangeNow() throws MalformedURLException, ApiException.ConnectionFailure {
     ...
     */
    @VisibleForTesting
    void trackChangeAt(final Instant instant) throws MalformedURLException, ApiException.ConnectionFailure {
        synchronized (this) {
            final int tolerance = RovasProperties.INACTIVITY_TOLERANCE;
     ...
     */
    public long commit() throws MalformedURLException, ApiException.ConnectionFailure {
            ...
            if (firstUncommitted == null || lastUncommitted == null) {
                ...
                if (Logging.isDebugEnabled()) {
                    Logging.debug(MessageFormat.format(
                            ...
                            firstUncommitted,
                            lastUncommitted,
                            RovasProperties.INACTIVITY_TOLERANCE
                    ));
                }
                this.committedSeconds = TimeConverterUtil.clampToSeconds(
                        this.committedSeconds +
                                ...
                                Math.min(
                                        ...
                                        Math.max(0, RovasProperties.INACTIVITY_TOLERANCE) // tolerance
                                )
                );
            }
        });
    ...
    public long uploadActionHappened(){
        Long lastUncommittedChange = Optional.ofNullable(this.lastUncommittedChangeTimestamp).orElse(0L);
        Long firstUncommittedChange = Optional.ofNullable(this.firstUncommittedChangeTimestamp).orElse(0L);
        return this.committedSeconds+
                Math.max(0, lastUncommittedChange - firstUncommittedChange) + // uncommitted time
                Math.min(
                        Math.max(0, Instant.now().getEpochSecond() - lastUncommittedChange), // time since last uncommitted timestamp
                        Math.max(0, RovasProperties.INACTIVITY_TOLERANCE) // tolerance
                );
    }
}
